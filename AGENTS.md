...
- `app/src/nodes/` - all LangGraph node callables, including search nodes, analyst nodes, cross-lane perspective synthesis, fact-checking, final composition, and supervisor rendering. The old `app/src/agents/` and `app/src/tools/` packages were merged into this package.
...
ingest_request -> build_research_plan
  ...
  -> synthesize_perspectives
     ...
     -> continue: fact_check -> compose_final -> supervisor -> END
```
...
- **State shape**: `PipelineState` stores `ResearchPlan`, `RefereeReport`, and `SynthesizedClaim` dataclass instances directly. Accumulating list fields such as source lists, fact checks, and synthesized claims use LangGraph `Annotated[..., operator.add]` reducers. The unused verification/rewrite loop state and old extracted-claims state were removed.
- **`synthesize_perspectives`** replaces the old referee + claim-extraction path. It cleans unsupported lane claims, checks loaded language, synthesizes consensus/contested/unique claims with cross-lane confidence, and can short-circuit through `referee_blocked_summary`.
- **`fact_check`** is the graph node backed by `nodes/cross_check_facts.py`; it checks synthesized claims with category/confidence context and returns weighted verdicts (`TRUE`, `PARTIALLY_TRUE`, `CONTESTED`, `UNVERIFIED`).
...
- **Search** (`app/src/search.py`) calls Brave Search, restricted to lane-allowed domains; it uses all LLM-planned queries, keeps up to two results per domain per query, deduplicates URLs lane-wide, and renumbers results with lane-prefixed source IDs (`L1`, `C1`, `R1`, `P1`, `F1`).
...
- `compose_final` depends on `synthesize_perspectives` not having blocked. If you change routing, also update the `_route_after_synthesis` conditional in `graph.py`.
