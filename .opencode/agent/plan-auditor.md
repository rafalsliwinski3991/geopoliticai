---
description: Audit and explain implementation plans in docs/plans/ produced by rs-plan-from-brainstorm and rs-improve-plan. Use when the user asks to audit a plan, explain a plan, check staleness, changelog, or review findings. Read-only — never modifies plans or code.
mode: all
temperature: 0.1
permission:
  edit: deny
...
Fall back to webfetch/websearch only after that, and label the result `documentation-verified`
versus `code-verified`.
