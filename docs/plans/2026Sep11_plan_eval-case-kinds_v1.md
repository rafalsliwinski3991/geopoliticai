...
`phoenix/client/resources/experiments/__init__.py:2826`: when `run_experiment`
...
| `app/tests/unit_tests/test_manual_quality_evaluation.py` | Loader validation, the shipped-file check, `route_correct`, and the new turn-loop check. |
...
command and the CI command run it as a script.
...
def build_kinds() -> dict[str, Kind]:
    ...
    return {
        ...
        # No `route_correct`: only the report branch pauses at a gate and
        # `run_report` already fails when the turn did not pause, so that judge
        # could never score anything but 1.0.
...
Validate: `.venv/bin/python -m mypy tests/manual_quality` and `-m ruff check .`
...
def thread_output(result: dict[str, Any]) -> dict[str, Any]:
    ...
    if destination not in {"geopolitical", "other", "report"}:
...
async def run_e2e(input: dict[str, Any]) -> dict[str, Any]:
    ...
    result: dict[str, Any] = {}
    for turn in turns:
        if not isinstance(turn, dict) or set(turn) not in ({"query"}, {"resume"}):
            raise ValueError("each turn needs exactly one of 'query' or 'resume'")
        if "query" in turn:
            query = turn["query"]
            # Validated here for the same reason `message_from_record` validates
            # its own fields: an empty query reaches Brave as an empty search
            # and returns as a confusing NoSourcesError several layers down.
            if not isinstance(query, str) or not query.strip():
                raise ValueError("each e2e turn query must be a non-empty string")
...
Validate: the new turn-loop unit test from the Test section, plus mypy and ruff.
...
- `route_correct` reads `route_correct_destination`, both the passing and the
  failing direction.
- `run_e2e` against a stub graph exposing only `ainvoke`: a `query` turn sends a
  `HumanMessage`, a `resume` turn sends a `Command`, a resume with no pending
  `__interrupt__` raises, a turn carrying neither key raises, and an empty or
  non-string query raises. No network, no application graph.
- `thread_output` excludes the final AI message from `conversation`. Feed it a
  ...
  contamination the review caught.
- `validate_evaluations` accepts a kind holding two cases. Build a fake result
  ...
  fails against the current `len(matching) != len(expected_names)` check.
...
5. **Accepted, low.** The plan claimed an orchestrator misroute to `report` is
   ...
   Verified at `langgraph/types.py:403,415`. The wording is corrected and both
   paths are now described.

Findings 1 and 4 are the reason two new regression tests were added: one
asserting the final message is absent from `conversation`, one asserting a
two-case kind validates.

## Open questions

- **The `report` kind dropping `route_correct`** was raised during the final
  design rather than put to the user as its own round. It is implemented here on
  the stated reasoning that the judge could only ever score 1.0. Say so if you
  want it kept anyway.
