...
The live code lives under `app/`. Repository-root `Dockerfile`, `main.py`, and `requirements.txt` are compatibility shims that delegate to `app/`; prefer `app/Dockerfile` and `app/src/*` for feature work. CI now installs/tests from `app/pyproject.toml` via uv.
...
- `frontend/` — single `index.html` (Alpine.js + `marked.js` + `DOMPurify` from CDN) plus `assets/`. No bundler. Served either directly by FastAPI (dev) or by nginx (prod).
...
- **API**: `app/src/api.py` exposes `POST /api/run_pipeline` (sync) and `POST /api/run_pipeline/stream` (SSE with per-node progress events). Both enforce an in-process token-bucket rate limit keyed by `X-Forwarded-For` (or `request.client.host`), while prod nginx also applies `limit_req` on `/api/`. Graphs are precompiled during FastAPI lifespan startup into a per-infosphere cache instead of being rebuilt per request. Requests may include optional `thread_id` for checkpointer isolation. Streaming uses `graph.astream_events(version="v2")` and emits Polish or English progress labels based on `infosphere`.
- **Frontend integration**: in dev, FastAPI mounts `/assets` and serves `frontend/index.html` at `/` via `FileResponse` (paths controlled by `FRONTEND_HTML_PATH`). In prod, nginx serves the static frontend and proxies `/api/` to the backend with TLS/basic auth and request throttling. Markdown output is sanitized with DOMPurify before rendering. `frontend/nginx.local.conf` exists but is not currently wired into any compose file.
...
- **Polish vs English prompts and sources are not interchangeable.** Both prompts and `INFOSPHERE_SOURCES` switch on runtime `language`/`infosphere` passed via `build_runtime_config()`.
- **Graphs are compiled at API startup** into a small cache (`english`/`polish`) and reused for sync + streaming requests.
- **`compose_final`** depends on referee not having blocked — if you change routing, also update `_route_after_referee` in `graph.py`.
- **CI installs from `app/pyproject.toml`** using `uv sync --project app --group dev`; keep dependency changes aligned there.


- Referee moderation criteria are documented in `docs/referee-policy.md`; update policy docs with any threshold/prompt changes.
