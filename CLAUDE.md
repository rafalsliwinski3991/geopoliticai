...
The live code lives under `app/`. The repository root `Dockerfile`, `main.py`, and `requirements.txt` are now compatibility shims that delegate to `app/` (they are no longer stale broken artifacts). The shipped image is still built from `app/Dockerfile`, and the primary CLI/API entrypoints remain in `app/src/`.
...
- **API**: `app/src/api.py` exposes `POST /api/run_pipeline` (sync) and `POST /api/run_pipeline/stream` (SSE with per-node progress events). Both enforce an in-process token-bucket rate limit keyed by `X-Forwarded-For` (or `request.client.host`). Graphs are precompiled at startup per infosphere and cached for reuse. Requests can include optional `thread_id` for checkpointer isolation. The streaming endpoint uses `graph.astream_events(version="v2")` and emits Polish or English progress labels based on the request's `infosphere`.
- **Frontend integration**: in dev, FastAPI mounts `/assets` and serves `frontend/index.html` at `/` via `FileResponse` (paths controlled by `FRONTEND_HTML_PATH`). In prod, nginx serves the static frontend and proxies `/api/` to the backend. `frontend/nginx.conf` is the prod config (TLS + basic auth via `docker-entrypoint.sh` writing `htpasswd`) and includes nginx-side API request throttling; `frontend/nginx.local.conf` exists but is not currently wired into any compose file.
...
- **Polish vs English prompts and sources are not interchangeable.** The curated `INFOSPHERE_SOURCES` and prompt language switch on runtime config (`language` / `infosphere`) provided per request.
- **Graphs are precompiled at startup per infosphere** and reused via cache in API endpoints. If graph construction changes, keep startup cache initialization and runtime config wiring aligned.
...
- Referee policy expectations are documented in `docs/referee-policy.md`; keep that document synchronized with prompt/threshold changes.
- CI installs from the `app` project (`uv sync --project app --group dev`) and runs unit tests from `app/tests/unit_tests`. Keep workflow and `app/pyproject.toml` aligned when changing dependencies.
