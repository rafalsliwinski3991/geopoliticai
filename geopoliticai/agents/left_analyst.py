...
def left_analyst_agent(
    ...
    source_block = "\n".join(_render_source_for_prompt(s) for s in state["left_sources"])
    allowed_source_ids = ", ".join(s.id for s in state["left_sources"]) or "none"
    reference_block = "\n".join(
        f"- {name} ({url})" for name, url in infosphere_sources["left"]
    )
    response_language = "Polish" if language == "polish" else "English"
    model_name = get_model("left_analyst")
    ...
    output = invoke_structured_chain(
        ...
        human_prompt=(
            ...
            "{reference_block}\n\n"
            "Task: Provide 3-5 analytically cautious claims from the perspective: leftist.\n"
            ...
            "{{\"claims\": [{{\"text\": \"According to L1, ...\", \"source_ids\": [\"L1\"]}}]}}\n"
            "Never return {{\"claims\": []}}."
        ),
        variables={
            "query": state["query"],
    ...
    if not claims:
        logger.info(
            "Left analyst: initial pass produced 0 claims from %d sources; retrying.",
        ...
        retry = invoke_structured_chain(
            ...
            human_prompt=(
                ...
                "Return JSON exactly like: {{\"claims\": [{{\"text\": \"According to L1, ...\", \"source_ids\": [\"L1\"]}}]}}. "
                "Never return {{\"claims\": []}}."
            ),
            variables={
                "query": state["query"],
        ...
        if not claims:
            logger.info("Left analyst: retry returned 0 claims; using source-note fallback.")
        ...
        if not claims:
            ...
            fallback_id = state["left_sources"][0].id if state["left_sources"] else ""
            claims = [Claim(text=f"Left perspective on: {state['query']}", source_ids=[fallback_id] if fallback_id else [])]
    logger.info("Left analyst: produced %d claims.", len(claims))
    for idx, claim in enumerate(claims, start=1):
        sources = ", ".join(claim.source_ids) if claim.source_ids else "none"
        logger.info(
            "Left claim %d/%d: %s (Sources: %s)",
            idx,
            len(claims),
            claim.text,
            sources,
