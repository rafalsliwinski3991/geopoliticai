...
def right_analyst_agent(
    ...
    source_block = "\n".join(_render_source_for_prompt(s) for s in state["right_sources"])
    allowed_source_ids = ", ".join(s.id for s in state["right_sources"]) or "none"
    reference_block = "\n".join(
        f"- {name} ({url})" for name, url in infosphere_sources["right"]
    )
    response_language = "Polish" if language == "polish" else "English"
    model_name = get_model("right_analyst")
    ...
    output = invoke_structured_chain(
        ...
        human_prompt=(
            ...
            "{{\"claims\": [{{\"text\": \"According to R1, ...\", \"source_ids\": [\"R1\"]}}]}}\n"
            "Never return {{\"claims\": []}}."
        ),
        variables={
            "query": state["query"],
    ...
    if not claims:
        logger.info(
            "Right analyst: initial pass produced 0 claims from %d sources; retrying.",
        ...
        retry = invoke_structured_chain(
            ...
            human_prompt=(
                ...
                "Return JSON exactly like: {{\"claims\": [{{\"text\": \"According to R1, ...\", \"source_ids\": [\"R1\"]}}]}}. "
                "Never return {{\"claims\": []}}."
            ),
            variables={
                "query": state["query"],
        ...
        if not claims:
            logger.info("Right analyst: retry returned 0 claims; using source-note fallback.")
        ...
        if not claims:
            ...
            fallback_id = state["right_sources"][0].id if state["right_sources"] else ""
            claims = [Claim(text=f"Right perspective on: {state['query']}", source_ids=[fallback_id] if fallback_id else [])]
