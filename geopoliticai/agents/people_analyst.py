...
def people_analyst_agent(
    ...
    source_block = "\n".join(_render_source_for_prompt(s) for s in state["people_sources"])
    allowed_source_ids = ", ".join(s.id for s in state["people_sources"]) or "none"
    reference_block = "\n".join(
    ...
    output = invoke_structured_chain(
        ...
        human_prompt=(
            ...
            "{{\"claims\": [{{\"text\": \"According to P1, ...\", \"source_ids\": [\"P1\"]}}]}}\n"
            "Never return {{\"claims\": []}}."
        ),
        variables={
            "query": state["query"],
    ...
    if not claims:
        ...
        retry = invoke_structured_chain(
            ...
            human_prompt=(
                ...
                "Return JSON exactly like: {{\"claims\": [{{\"text\": \"According to P1, ...\", \"source_ids\": [\"P1\"]}}]}}. "
                "Never return {{\"claims\": []}}."
            ),
            variables={
                "query": state["query"],
        ...
        if not claims:
            ...
            fallback_id = state["people_sources"][0].id if state["people_sources"] else ""
            claims = [Claim(text=f"People perspective on: {state['query']}", source_ids=[fallback_id] if fallback_id else [])]
