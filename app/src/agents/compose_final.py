...
def _fallback_synthesis(state: PipelineState, language: str) -> str:
    ...
    for author, claim in claim_with_author:
        ...
        if text in claim_verdicts:
            ...
            claim_lines.append(
                f"- [{author}] {text}\n"
                f"  Verdict: {verdict} | {rationale} | Sources: {sources}"
            )
        else:
            claim_lines.append(
                f"- [{author}] {text}\n"
...
def compose_final_agent(state: PipelineState, language: str) -> PipelineState:
    ...
    data = invoke_structured_chain(
        ...
        human_prompt=(
            ...
            "- Use only facts present in the provided claims and fact-check results.\n"
            ...
            "- Do not add outside sources (for example Wikipedia) or uncited external knowledge.\n"
